...
Direct messages and explicit mentions flush immediately. P2P identity uses the sender `open_id`, so proactive Huly delivery and later direct messages resume one Session even though Feishu uses a different `chat_id` for the direct chat. Incoming resources are downloaded to owner-only local state before the turn. Downloaded images are also committed to Harness attachment storage and appended to that user message as native image blocks; their local paths remain available for tools. Completed inbox IDs remain durable for twelve hours across process restarts.
